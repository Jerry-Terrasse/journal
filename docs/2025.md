...
在Clash Verge Rev中添加全局拓展覆写配置，以让特定目的IP使用常规路由。

```yaml
tun:
  route-exclude-address:
    - 233.233.233.233/32 # example ip
```
...
使用[SingleFile浏览器插件](https://www.getsinglefile.com/)可以将网页保存为单HTML文件，并基本保存所有可见内容及样式完整。
...
[Vimprove - Interactive Vim Training](https://vim.terras.su)
